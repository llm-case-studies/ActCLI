...
def run_wizard() -> None:
    ...
    # Trust
    trust_choice = typer.prompt(
        "Trust this folder? (persist/once/deny)", default="persist"
    )
    ...
    # Models
    models = typer.prompt(
        "Default models (comma-separated)", default="llama3,claude,gpt"
    )
    # Cloud share
    cs = typer.confirm(
        "Allow sharing file content to cloud models by default?", default=False
    )
    ...
    cfg = Config(
        project_name=None,
        project_version="0.1",
        defaults=Defaults(mode=mode, output_dir=outdir, models=models),
    )
    ...
    if trust_choice in ("persist", "once"):
        set_trust(
            None, trust_choice, read=["./**"], write=[f"./{outdir}/**"], cloud_share=cs
        )
        console.print(
            Panel(f"Trusted this folder ({trust_choice}).", border_style="green")
        )
    else:
        console.print(
            Panel("Not trusting this folder; reads limited.", border_style="yellow")
        )
