...
def run_trust(action: str, scope: Optional[str], cloud_share: Optional[bool]) -> None:
    ...
    if action == "status":
        ...
        if not tr:
            console.print(
                Panel(
                    "Untrusted. Use 'actcli trust allow-here' to trust this folder.",
                    title="Trust",
                    border_style="yellow",
                )
            )
            ...
            lines = [
                ...
                f"scope: {tr.scope}",
                f"read: {', '.join(tr.read)}",
                f"write: {', '.join(tr.write)}",
                f"cloud_share: {tr.cloud_share}",
            ]
    ...
    if action in ("allow-here", "allow-once"):
        ...
        console.print(
            Panel(
                f"Trusted this folder (scope={scope_val}, cloud_share={cs}).",
                border_style="green",
            )
        )
        return
