...
from ..schemas.mcp_runtime import ToolMeta, RpcRequest, RpcAccepted, CancelRequest, CancelResponse
...
def mcp_rpc_route(req: RpcRequest) -> RpcAccepted:
    ...
    return RpcAccepted(ok=True, job_id=jr.id, note="MVP: worker not yet implemented; use /mcp/sse for stub stream")
...
def mcp_sse_route(job: str):
    ...
    def _gen():
        ...
        if jr is None:
            ...
            return
        # Route to tool-specific stream; MVP supports only excel.inspect
        if jr.tool == 'excel.inspect':
            ...
            fault = {"event": "fault", "job": job, "ok": False, "error": f"unsupported tool: {jr.tool}"}
            yield f"data: {json.dumps(fault)}\n\n"
