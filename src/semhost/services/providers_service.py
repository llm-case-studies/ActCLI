...
def _auth_probe_codex() -> Tuple[str, str]:
    ...
    try:
        ...
        _ = _version("codex")
        p = subprocess.run(
            ["codex", "exec", "ping"], capture_output=True, text=True, timeout=t
        )
        if p.returncode == 0:
            return ("ok", "signed in")
...
def _auth_probe_gemini() -> Tuple[str, str]:
    ...
    try:
        p = subprocess.run(
            ["gemini", "whoami"], capture_output=True, text=True, timeout=5
        )
    ...
    try:
        ...
        _ = _version("gemini")
        p2 = subprocess.run(
            ["gemini", "-p", "test"], capture_output=True, text=True, timeout=t
        )
        if p2.returncode == 0:
            return ("ok", "signed in")
...
def providers_login(req: CliLoginRequest) -> CliLoginResponse:
    ...
    if req.provider == "codex_cli":
        if not _which("codex"):
            return CliLoginResponse(
                launched=False, hint="codex binary not found; npm i -g @openai/codex"
            )
        try:
            # Non-blocking spawn; user completes interactively in terminal
            stderr = None if not st.cli_debug else None
            subprocess.Popen(
                ["codex"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
    ...
    if req.provider == "claude_cli":
        if not _which("claude"):
            return CliLoginResponse(
                launched=False,
                hint="claude binary not found; npm i -g @anthropic-ai/claude-code",
            )
        try:
            subprocess.Popen(
                ["claude"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            return CliLoginResponse(launched=True)
        except Exception as e:
            return CliLoginResponse(launched=False, hint=str(e))
    ...
    if req.provider == "gemini_cli":
        if not _which("gemini"):
            return CliLoginResponse(
                launched=False,
                hint="gemini binary not found; npm i -g @google/gemini-cli@nightly",
            )
        try:
            subprocess.Popen(
                ["gemini"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            return CliLoginResponse(launched=True)
        except Exception as e:
            return CliLoginResponse(launched=False, hint=str(e))
...
def providers_switch_model(req: CliModelSwitchRequest) -> CliModelSwitchResponse:
    ...
    try:
        if prov == "codex_cli":
            ...
            p = subprocess.run(
                ["codex", "/model", model], capture_output=True, text=True, timeout=8
            )
            if p.returncode == 0:
        ...
        if prov == "claude_cli":
            ...
            p = subprocess.run(
                ["claude", "-p", "test", "--output-format", "json", "--model", model],
                capture_output=True,
                text=True,
                timeout=8,
            )
