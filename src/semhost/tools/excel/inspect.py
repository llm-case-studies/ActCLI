...
def stream(
    ...
    if not src.exists() or not src.is_file():
        yield {
            ...
            "job": job_id,
            "ok": False,
            "error": "input not found or not a file",
        }
        return
    ...
    try:
        ...
        if not str(real).startswith(str(ro)):
            yield {
                ...
                "job": job_id,
                "ok": False,
                "error": "path must be under /mnt/ro",
            }
            return
    except Exception:
        pass
    ...
    if (time.time() - t0) > 0.8 * budget_s and not warned_80:
        yield {
            "event": "warning",
            "job": job_id,
            "msg": "Approaching time budget (80%)",
        }
        warned_80 = True
            ...
            with zipfile.ZipFile(src, "r") as z:
                ...
                if len(names) > 10000:
                    yield {
                        ...
                        "job": job_id,
                        "ok": False,
                        "error": "zip too large (members > 10000)",
                    }
                    return
                total_uncomp = 0
                ...
                if "EncryptionInfo" in names:
                    ...
                    if msoffcrypto is None:
                        yield {
                            ...
                            "job": job_id,
                            "ok": False,
                            "error": "PASSWORD_PROVIDED_BUT_MSOFFCRYPTO_MISSING",
                        }
                        return
    ...
    if load_workbook is not None and suffix in (".xlsx", ".xlsm"):
        ...
        yield {
            ...
            "job": job_id,
            "pct": 55,
            "msg": "Scanning formulas (openpyxl)",
        }
        try:
    ...
    try:
        if "vba_bins" in locals() and VBA_Parser is not None and vba_bins:
            ...
            proc_re = re.compile(
                r"^\s*(Sub|Function)\s+([A-Za-z0-9_]+)\s*\(",
    ...
    yield {
        ...
        "job": job_id,
        "pct": 80,
        "msg": "Computing hashes & writing reports",
    }
    ...
    score = (
        (3 if flags["has_macros"] else 0)
        + (2 if flags["external_links"] else 0)
        + (1 if flags["connections"] else 0)
        + (1 if vol_ratio > 0.02 else 0)
    )
    severity = "green"
    if score >= 4:
        severity = "red"
    ...
    pre_md = [
        ...
        f"- Kind: {kind}",
        f"- Sheets: {ws_count}",
        f"- Macros: {'yes' if has_vba else 'no'}",
        f"- External links: {len(external_links)}",
        f"- Connections: {'yes' if connections_present else 'no'}",
        f"- Severity: {severity}",
    ]
    if notes:
    ...
    if parts_dir.exists():
        for p in parts_dir.rglob("*"):
            if p.is_file():
                artifacts.append(p)
    artifact_entries = [
        {"path": str(_safe_rel(out_root, a)), "sha256": _sha256_file(a)}
        for a in artifacts
    ]
    ...
    try:
        ...
        params_hash = hashlib.sha256(
            json.dumps(params_norm, sort_keys=True).encode("utf-8")
        ).hexdigest()
        job = {
            "tool": "excel.inspect",
            "job_id": job_id,
