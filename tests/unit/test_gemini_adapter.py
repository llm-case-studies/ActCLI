...
def test_gemini_generate(monkeypatch) -> None:
    ...
    respx.post(
        "https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash-latest:generateContent?key=gk"
    ).respond(json={"candidates": [{"content": {"parts": [{"text": "ok"}]}}]})
    a = GeminiAdapter()
    out = a.generate("q")
    assert out == "ok"
...
def test_gemini_error(monkeypatch) -> None:
    ...
    respx.post(
        "https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash-latest:generateContent?key=gk"
    ).respond(status_code=403, json={"error": {"message": "denied"}})
    a = GeminiAdapter()
    try:
        a.generate("q")
